...
- **Team ([company.html#team](company.html#team))** — no headshots/bios were supplied. The grid shows "Name Pending / Role Pending" placeholder cards.
...
- **Case studies ([projects.html](projects.html))** — the source material gave client *logos* but no named, attributable projects. Three "illustrative example" vignettes are used instead, explicitly labeled as not tied to a specific client. Real case studies (with a client quote and project photo) would meaningfully strengthen this page.
...
- **Imagery.** No usable project photography files were provided (the source PDF described photos but they weren't extractable as image assets). Rather than use generic/unlicensed stock photography, hero and section backgrounds use an original abstract topographic-line motif in the brand colors — fitting for a GIS company, and it keeps the site fast and dependency-free. Swap in real Phirez photography (drone demos, field GNSS work, technician at workstation, etc.) whenever it's available — the `.hero-bg` / `.topo-bg` containers are built to take a plain `<img>` in place of the SVG pattern.
...
- **Navigation.** The brief called for two separate nav systems (a desktop full-screen overlay vs. a mobile accordion-in-header). This build uses one unified full-screen overlay with mega-panels (same interaction pattern as Optron) that responsively reflows its grid at every breakpoint — same UX, less code to maintain, and it stays keyboard- and screen-reader-operable throughout.
...
- **Stack:** plain HTML/CSS/JS, no build step. Fonts (Google Fonts), the carousel (Swiper.js) and the map load from public CDNs at runtime — an internet connection is required (no offline fallback bundled, which is standard for a site like this).
...
- [ ] Swap in real project photography and the official logo file, if/when available
